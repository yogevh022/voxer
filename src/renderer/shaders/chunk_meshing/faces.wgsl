...
fn mesh_chunk_position(blocks: ptr<function, ChunkBlocks>, x: u32, y: u32) {
    ...
    for (var z: u32 = 0u; z < CHUNK_DIM_HALF; z++) {
        let current: u32 = (*blocks)[x][y][z];
        let next_x: u32 = select(0u, (*blocks)[x+1u][y][z], x < (CHUNK_DIM - 1));
        ...
        let next_y: u32 = select(0u, (*blocks)[x][y+1u][z], y < (CHUNK_DIM - 1));
        ...
        let next_z: u32 = select(0u, (*blocks)[x][y][z + 1u], z < (CHUNK_DIM_HALF - 1));
