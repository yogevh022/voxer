...
@group(0) @binding(1)
var<storage, read_write> indirect_dispatch_buffer: GPUIndirectArgsAtomic;
@group(0) @binding(2)
...
fn write_culled_mdi(
    ...
    let chunk_index = chunks_view_buffer[mdi_arg_index].index;
    let chunk_header: GPUVoxelChunkHeader = chunks_meta_buffer[chunk_index];
...
fn update_local_fid_draws(chunk_header: GPUVoxelChunkHeader, camera_chunk_pos: vec3<i32>, draw_mask: u32) {
    ...
    let fid_offsets = mesh_entry_face_offsets(chunk_header.face_alloc, fid_counts);
