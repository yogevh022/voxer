...
const MAX_WG_INDIRECT_DRAW_ARGS = CFG_MAX_WORKGROUP_DIM_1D + VOID_OFFSET;
var<workgroup> wg_indirect_draw_args: array<GPUDrawIndirectArgs, MAX_WG_INDIRECT_DRAW_ARGS>;
...
fn write_culled_mdi(
    ...
    let visible_fid_masks = visible_face_id_masks(camera_position, chunk_position_f32);
    ...
    if exists && within_render_distance && within_view_frustum {
        let face_offsets_with_counts = unpack_mesh_face_offsets_with_counts(mesh_entry);
        var arg_idx = atomicAdd(&wg_indirect_draw_args_count, visible_fid_masks.count);
        for (var fid = 0u; fid < 6u; fid++) {
            let draw_args = GPUDrawIndirectArgs(
                face_offsets_with_counts.counts[fid] * 6u,  // vertex_count
                1u,                                         // instance_count
                0u,                                         // first_vertex
                face_offsets_with_counts.offsets[fid] * 6u, // first_instance
            );
            wg_indirect_draw_args[visible_fid_masks.masks[fid] * (arg_idx + VOID_OFFSET)] = draw_args;
            arg_idx += visible_fid_masks.masks[fid];
        }
    }
    workgroupBarrier();
...
fn visible_face_id_masks(camera_position: vec3<f32>, chunk_position_f32: vec3<f32>) -> VisibleFaceIDs {
    let camera_chunk_position_f32: vec3<f32> = floor(camera_position / f32(CHUNK_DIM));
    let draw_px = u32(chunk_position_f32.x <= camera_chunk_position_f32.x);
    let draw_mx = u32(chunk_position_f32.x >= camera_chunk_position_f32.x);
    let draw_py = u32(chunk_position_f32.y <= camera_chunk_position_f32.y);
    let draw_my = u32(chunk_position_f32.y >= camera_chunk_position_f32.y);
    let draw_pz = u32(chunk_position_f32.z <= camera_chunk_position_f32.z);
    let draw_mz = u32(chunk_position_f32.z >= camera_chunk_position_f32.z);

//    let masks = array<u32, 6>(draw_px, draw_mx, draw_py, draw_my, draw_pz, draw_mz);
    let masks = array<u32, 6>(1, 0, 1, 0, 1, 0);
//    let count = draw_px + draw_mx + draw_py + draw_my + draw_pz + draw_mz;
    let count = 3u;

    return VisibleFaceIDs(masks, count);
}
