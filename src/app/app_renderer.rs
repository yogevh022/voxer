...
impl AppRenderer<'_> {
    pub fn write_new_chunks(&mut self, chunks: Vec<(usize, IVec3, Chunk)>) {
        ...
        for (slab_index, chunk_pos, chunk) in chunks.into_iter() {
            ...
            let header = GPUChunkEntryHeader::new(
                self.gpu_vertex_malloc.alloc(vertex_count).unwrap() as u32,
                self.gpu_index_malloc.alloc(index_count).unwrap() as u32,
                vertex_count as u32,
                index_count as u32,
                slab_index as u32,
        ...
        self.renderer.write_buffer(&self.chunk_buff, 0, &storage_buffer.into_inner());
    ...
    pub fn compute_chunks(&mut self) {
        ...
        compute_pass.dispatch_workgroups(self.gpu_loaded_chunk_entries.len() as u32, 1, 1);
    }
    ...
    pub fn encode_render_pass(&mut self, view: &wgpu::TextureView, camera: &vtypes::Camera) {
        ...
        for header in self.gpu_loaded_chunk_entries.values() {
            indirect_buffer_commands.push(DrawIndexedIndirectArgs {
                index_count: header.index_count,
                instance_count: 1,
                first_index: header.index_offset,
                base_vertex: header.vertex_offset as i32,
                first_instance: header.slab_index,
            });
        }
