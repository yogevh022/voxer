...
pub fn write_texture_to_atlas(tex_data: &TextureData, atlas: &mut TextureAtlas) {
    let tex_image = image::open(get_image_path(tex_data.source))
        .unwrap()
        .to_rgba8();
    let tex_index = tex_data.kind as u32;
    let x_offset = TEXTURE_DIM as f32 * ((tex_index % atlas.cell_dim) as f32);
    let y_offset = TEXTURE_DIM as f32 * (tex_index as f32 / atlas.cell_dim as f32).floor();
